...
static void usage(const char *message)
{
    ...
    fprintf(stderr, R"TXT(Usage:
%s [options] [filenames]

This program is dedicated to GxScript

Options:
    -l
        List (use -l -l for full listing)
    -o name
        Output to file 'name' (default is "%s")
    -v
...
{
    UNUSED(L);
    GByteArray &buffer = *reinterpret_cast<GByteArray *>(u);
    buffer.write(p, (int64_t) size);
    return 0;
}
...
static int pmain(lua_State *L)
{
    ...
    if (dumping) {
        ...
        lua_lock(L);
        luaU_dump(L, f, writer, (void *) &buffer, 0);
        lua_unlock(L);

        GByteArray outData;
        const char head[4] = {(char) 0xff, 'l', 's', (char) 0xee};
        outData.write(head, 4);
        outData << GByteArray::compress(buffer);

        GFile outFile(output);
        if (outFile.open(GFile::WriteOnly | GFile::Binary)) {
